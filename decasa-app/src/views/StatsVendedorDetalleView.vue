...
import { getStatsVendedor, getTendencia } from '@/api/stats'
...
const vendedorId = Number(route.params.id)
...
async function cargar() {
  ...
  try {
    const p = params()
    const sRes = await getStatsVendedor(vendedorId, p)
    stats.value = sRes.data
    const tRes = await getTendencia({ ...p, vendedor_id: vendedorId })
...
<template>
  <div class="p-4 max-w-2xl mx-auto space-y-4 pb-8">
    ...
    <div class="flex items-center gap-3">
      <button @click="router.back()" class="p-1.5 rounded-lg hover:bg-gray-100 transition-colors">
        <ArrowLeftIcon class="w-5 h-5 text-gray-600" />
      </button>
      <div class="flex-1">
        <h2 class="text-lg font-bold text-gray-800">{{ stats?.vendedor?.nombre ?? 'Estadísticas vendedor' }}</h2>
        <p v-if="stats?.vendedor?.tienda" class="text-xs text-gray-400 mt-0.5">{{ stats.vendedor.tienda }}</p>
      </div>
    </div>
    ...
    <template v-else-if="stats">
      ...
      <div class="grid grid-cols-2 gap-3">
        ...
        <div class="bg-white rounded-xl shadow-sm p-4">
          ...
          <p class="text-[11px] text-gray-400 mt-1">Saldo vivo de sus órdenes a hoy, sin filtro de período.</p>
        </div>
      </div>
      ...
      <div v-if="stats.comparativa_equipo" class="bg-white rounded-xl shadow-sm p-4">
        ...
        <div class="grid grid-cols-2 gap-3 text-center">
          ...
          <div class="bg-gray-50 rounded-xl py-3 px-2">
            <p class="text-lg font-bold text-gray-700">{{ cop(stats.comparativa_equipo.promedio_ingresos) }}</p>
            <p class="text-xs text-gray-500 mt-0.5">Promedio del equipo</p>
          </div>
        </div>
        <p class="text-xs text-gray-400 mt-2 text-center">
          Equipo: {{ stats.comparativa_equipo.num_vendedores }} vendedores · Total: {{ cop(stats.comparativa_equipo.total_equipo) }}
        </p>
      </div>
